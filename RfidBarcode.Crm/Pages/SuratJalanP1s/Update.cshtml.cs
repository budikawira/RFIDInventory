...
namespace RfidBarcode.Crm.Pages.SuratJalanP1s
{
    public class UpdateModel : BasePageModel
    {
        ...
        {
            var cmd = new UnfinalizeSuratJalanRequest(SuratJalanP1Id);
            var response = await _mediator.Send(cmd);
        ...
        public async Task<IActionResult> OnPostSelect2LocationOutbondAsync()
        {
            ...
            {
                Type = Location.TYPE_END_LOCATION,
                Skip = 0,
                PageSize = 100,
                SearchValue = search
            };
