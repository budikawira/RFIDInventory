...
{
    public class AddItemForP1Handler : BaseHandler, IRequestHandler<AddItemsForP1Request, BaseResponse>
    {
        public AddItemForP1Handler(IApplicationDbContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }

        public async Task<BaseResponse> Handle(AddItemsForP1Request request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                if (items.Count == request.ItemIds.Count)
                {
                    ...
                    {
                        item.SuratJalanP1Id = request.SuratJalanP1Id;
                    }
