...
{
    public class GetAllItemSummaryForOutbondQuery : BaseHandler, IRequestHandler<GetAllItemSummaryForOutbondRequest, BaseDataTableResponse<ItemSummaryForOutbondVM>>
    {
        public GetAllItemSummaryForOutbondQuery(IApplicationDbContext context, IMapper mapper)
        {
            _context = context;
            _mapper = mapper;
        }

        public async Task<BaseDataTableResponse<ItemSummaryForOutbondVM>> Handle(GetAllItemSummaryForOutbondRequest request, CancellationToken cancellationToken)
        {
            var response = new BaseDataTableResponse<ItemSummaryForOutbondVM>()
            {
                Draw = request.Draw
            };
            ...
            try
            {
                var query = _context.Items
                    ...
                    .Select(g => new ItemSummaryForOutbondVM
                    {
                        //Kp = g.Key.Kp,
                        Kode = g.Key.Kode,
                        Kode1 = g.Key.Kode1,
